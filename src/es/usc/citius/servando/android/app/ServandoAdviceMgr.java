...
public class ServandoAdviceMgr {
	...
	public interface AdviceListener {
		public void onAdvice(Advice advice);
	}

	private static final ServandoAdviceMgr instance = new ServandoAdviceMgr();

	private List<AdviceListener> listeners;

	private ServandoAdviceMgr()
	{

		listeners = new ArrayList<ServandoAdviceMgr.AdviceListener>();
		...
		Advice a6 = new Advice("medico", "A túa cita do mércores queda confirmada para as 10:15h. <br/> Saúdos :)", d3);
	...
	public void setHomeAdvice(Advice homeAdvice)
	{
		...
		for (AdviceListener l : listeners)
		{
			l.onAdvice(homeAdvice);
		}
	}

	public void addAdviceListener(AdviceListener l)
	{
		if (!listeners.contains(l))
			listeners.add(l);
	}

	public void removeAdviceListener(AdviceListener l)
	{
		if (listeners.contains(l))
			listeners.remove(l);
	}
